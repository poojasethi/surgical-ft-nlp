...
class ProtoNetNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        for _ in range(NUM_CONV_LAYERS):
            layers.append(nn.Conv2d(in_channels, NUM_HIDDEN_CHANNELS, (KERNEL_SIZE, KERNEL_SIZE), padding="same"))
...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # ******************* YOUR CODE HERE *********************
            # ********************************************************
        return (torch.mean(torch.stack(loss_batch)), np.mean(accuracy_support_batch), np.mean(accuracy_query_batch))
    ...
    def train(self, dataloader_train, dataloader_val, writer):
        ...
        for i_step, task_batch in enumerate(dataloader_train, start=self._start_train_step):
            ...
            if i_step % PRINT_INTERVAL == 0:
                ...
                writer.add_scalar("train_accuracy/support", accuracy_support.item(), i_step)
            ...
            if i_step % VAL_INTERVAL == 0:
                with torch.no_grad():
                    ...
                    for val_task_batch in dataloader_val:
                        loss, accuracy_support, accuracy_query = self._step(val_task_batch)
    ...
    def _save(self, checkpoint_step):
        ...
        """
        torch.save(
            dict(network_state_dict=self._network.state_dict(), optimizer_state_dict=self._optimizer.state_dict()),
...
def main(args):
    ...
    if not args.test:
        ...
        protonet.train(dataloader_train, dataloader_val, writer)
    else:
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=0.001, help="learning rate for the network")
    parser.add_argument("--batch_size", type=int, default=16, help="number of tasks per outer-loop update")
    parser.add_argument(
        "--num_train_iterations", type=int, default=5000, help="number of outer-loop updates to train for"
    )

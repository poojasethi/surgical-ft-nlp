...
class MAML:
    ...
    def __init__(self, num_outputs, num_inner_steps, inner_lr, learn_inner_lrs, outer_lr, log_dir):
        """Inits MAML.
        ...
        for i in range(NUM_CONV_LAYERS):
            meta_parameters[f"conv{i}"] = nn.init.xavier_uniform_(
                torch.empty(
                    NUM_HIDDEN_CHANNELS, in_channels, KERNEL_SIZE, KERNEL_SIZE, requires_grad=True, device=DEVICE
                )
            )
        ...
        meta_parameters[f"w{NUM_CONV_LAYERS}"] = nn.init.xavier_uniform_(
            torch.empty(num_outputs, NUM_HIDDEN_CHANNELS, requires_grad=True, device=DEVICE)
        )
        meta_parameters[f"b{NUM_CONV_LAYERS}"] = nn.init.zeros_(
            torch.empty(num_outputs, requires_grad=True, device=DEVICE)
        )

        self._meta_parameters = meta_parameters
        self._num_inner_steps = num_inner_steps
        self._inner_lrs = {
            k: torch.tensor(inner_lr, requires_grad=learn_inner_lrs) for k in self._meta_parameters.keys()
        }
        self._outer_lr = outer_lr

        self._optimizer = torch.optim.Adam(
            list(self._meta_parameters.values()) + list(self._inner_lrs.values()), lr=self._outer_lr
        )
    ...
    def _forward(self, images, parameters):
        ...
        return F.linear(input=x, weight=parameters[f"w{NUM_CONV_LAYERS}"], bias=parameters[f"b{NUM_CONV_LAYERS}"])
    ...
    def train(self, dataloader_train, dataloader_val, writer):
        ...
        for i_step, task_batch in enumerate(dataloader_train, start=self._start_train_step):
            ...
            if i_step % LOG_INTERVAL == 0:
                ...
                writer.add_scalar("train_accuracy/pre_adapt_support", accuracies_support[0], i_step)
                writer.add_scalar("train_accuracy/post_adapt_support", accuracies_support[-1], i_step)
                writer.add_scalar("train_accuracy/post_adapt_query", accuracy_query, i_step)
            ...
            if i_step % VAL_INTERVAL == 0:
                ...
                for val_task_batch in dataloader_val:
                    outer_loss, accuracies_support, accuracy_query = self._outer_step(val_task_batch, train=False)
                ...
                writer.add_scalar("val_accuracy/pre_adapt_support", accuracy_pre_adapt_support, i_step)
                writer.add_scalar("val_accuracy/post_adapt_support", accuracy_post_adapt_support, i_step)
                writer.add_scalar("val_accuracy/post_adapt_query", accuracy_post_adapt_query, i_step)
...
def main(args):
    ...
    maml = MAML(args.num_way, args.num_inner_steps, args.inner_lr, args.learn_inner_lrs, args.outer_lr, log_dir)
    ...
    if not args.test:
        ...
        dataloader_test = omniglot.get_omniglot_dataloader(
            "test", 1, args.num_way, args.num_support, args.num_query, NUM_TEST_TASKS
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_support", type=int, default=1, help="number of support examples per class in a task")
    ...
    parser.add_argument(
        "--learn_inner_lrs", default=False, action="store_true", help="whether to optimize inner-loop learning rates"
    )
    parser.add_argument("--outer_lr", type=float, default=0.001, help="outer-loop learning rate")
    parser.add_argument("--batch_size", type=int, default=16, help="number of tasks per outer-loop update")
    parser.add_argument(
        "--num_train_iterations", type=int, default=15000, help="number of outer-loop updates to train for"
    )

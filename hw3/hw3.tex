...
\begin{enumerate}
    ...
    \textbf{You can expect accuracy numbers \textit{roughly} in the 0.5-0.9 range for bAbI QA in this assignment.}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item \texttt{qa} \textbf{[only for bAbI]}: Add ``\texttt{ In the }'' after the question and before each answer, since this task involves answering questions about the physical whereabouts of a person. In addition, add a period after the answer (omitting the period can significantly impact your results!). Be sure to include a space between the question and \texttt{In the}, as well as a space before the answer (though keep in mind Note 1!).
            ...
            \item \texttt{tldr} \textbf{[only for XSum]}: Add the text ``\texttt{TL;DR: }'' after the article/input and before the summary/target.
...
\begin{enumerate}
    ...
    \end{enumerate}
    This step simply requires selecting the correct subset of parameters for each version listed above in \texttt{parameters\_to\_fine\_tune()}.
